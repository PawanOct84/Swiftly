...
//  ModelGenerator
//
//  Created by Pawan Kumar on 25/07/2021.
...
public struct ModelGenerator {
    ...
    func generate() -> [ModelFile] {
        return addModelForJSON(baseContent, configuration.baseClassName, true)
    }
            ...
            for (key, value) in rootObject {
                ...
                case .array:
                    if value.arrayValue.isEmpty {
                        ...
                        if subClassType == .object {
                            var models = generateModelForJSON(JSONHelper.reduce(value.arrayValue), variableName, false)
                            modelFiles += models
    ...
    func addModelForJSON(_ object: JSON, _ defaultClassName: String, _ isTopLevelObject: Bool) -> [ModelFile] {
        let className = NameGenerator.fixClassName(defaultClassName, configuration.prefix, isTopLevelObject)
        var modelFiles: [ModelFile] = []
        ...
        if let rootObject = object.array, let firstObject = rootObject.first {
            ...
            // stuff. However, currently it does not make a base file to handle the array.
            if subClassType == .object {
                return addModelForJSON(JSONHelper.reduce(rootObject), defaultClassName, isTopLevelObject)
            }
            return []
        }
            ...
            for (key, value) in rootObject {
                ...
                case .array:
                    if value.arrayValue.isEmpty {
                        ...
                        let subClassType = value.arrayValue.first!.detailedValueType()
                        if subClassType == .object {
                            var models = addModelForJSON(JSONHelper.reduce(value.arrayValue), variableName, false)
                            modelFiles += models
                            let model = models.first
                            let classname = model?.fileName
...
//        notification.title = NSLocalizedString("ModelGenerator", comment: "")
